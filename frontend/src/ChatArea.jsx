...
export default function ChatArea(attrs) {
    const { chatArea, setChatArea } = attrs.vars
    ...
    return (chatArea ?
        <div className={`relative flex flex-col h-[100vh] w-[calc(100vw-397px)] min-w-[277px]`}>
            <div className={`${numSelected ? 'bg-[#ffffff11]' : ''} px-4 flex justify-between items-center w-full min-h-[75px] border-b border-borders`}>
                {!numSelected ? (
                    <><div className='gap-4 flex items-center'>
                        ...
                        <div className='hidden lg:block md:hidden sm:hidden text-white text-[1.2rem] font-semibold'>{chatArea.User.username}</div>
                    </div>
                    <div className='flex gap-4 items center justify-self-end'>
                        <button className='active:opacity-55 cursor-pointer'><VoiceChatIcon/></button>
            ...
            <div ref={chatAreaRef} className='pt-3 flex flex-col gap-2 overflow-y-auto w-full h-full'>
                {/* {arr.map((item, index) => (
                        ...
                        }} key={index} className={`px-5 py-2 ${numSelected ? 'cursor-pointer' : ''} ${item.selected ? 'bg-[#ffffff2d]' : ''} w-full group relative flex flex-col ${item.sender == 'Bob' ? 'items-start self-start' : 'items-end self-end'}`}>
                            <h1 style={{
                                wordBreak: 'break-word',
                                whiteSpace: 'pre-wrap',
                                overflowWrap: 'break-word',
                            }} className={`flex items-center relative font-[100] px-4 p-3 text-left text-[.8rem] max-w-[350px] bg-[#3797F0] text-white rounded-2xl ${item.sender == 'Bob' ? 'rounded-bl-none' : 'rounded-br-none'}`}>
                                <button onClick={() => {
                                    setNumSelected(prev => prev + 1)
                                    setArr(prev => 
                                        prev.map((item, i) => 
                                ...
                                }} className={`hidden ${numSelected ? '' : 'group-hover:block'} absolute ${item.sender == 'Bob' ? 'right-1' : '-left-[2.3rem]'} text-[.8rem] font-normal text-[#ffffff7a] hover:text-white`}>More</button>
                                {item.message}
                            </h1>
            ...
            <div className={`pt-10 flex flex-col gap-2 items-center transition ease-in-out duration-500 ${openInfo ? 'translate-x-0' : 'translate-x-[382px]'} absolute right-0 h-full w-[382px] border-l border-borders bg-black text-white`}>
                ...
                <div className='text-[2rem]'>{chatArea.User.username}</div>
                <div className='font-[200] mt-7 flex flex-col w-[80%] gap-2 text-[.9rem]'>
                    ...
                    <div className='opacity-70 flex items-center w-[80%] gap-2 text-[.9rem]'>
                        <Calendar/>
                        <h1>Account created on <b className='font-bold'>15 July 2024</b></h1>
                    </div>
                    ...
                    <div className='px-[1px] flex items-center w-[80%] gap-2 text-[.9rem]'>
                        <Mutuals/>
                        <h1 className='opacity-70'>Mutual Friends &nbsp;<b>-</b></h1><h1 className='font-bold'>12</h1>
                    </div>
                </div>
                <div className='mt-14 w-[80%] gap-1 items-start flex flex-col font-[100] text-[1.2rem] text-red-500'>
                    ...
                    <button className='underline-offset-4 hover:underline'>Block <b className='font-semibold'>{chatArea.User.username}</b></button>
                </div>
            </div>
        </div>
        :
        <div className='flex flex-col gap-1 items-center justify-center h-[100vh] w-full text-white'>
            <div><ChatIcon/></div>
            <div className='text-[4vh] text-[#ffffff57] font-light text-nowrap'>No opened chat</div>
        </div>
    )
}