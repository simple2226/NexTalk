        ...
        <div className='pt-5 flex flex-col items-start h-[100vh] min-w-[397px] border-r border-borders'>
            ...
            {data ?                
                (data.Contacts.sort((a, b) => new Date(b.lastUpdated) - new Date(a.lastUpdated)).map((item, index) => {
                    return <div onClick={() => setChatInfo({ chat_id: item._id, my_id: account._id, user_id: item.others_id })} key={index} className='relative px-4 gap-3 flex items-center w-full min-h-[75px] hover:bg-[#ffffff17] cursor-pointer'>
                        ...
                        <div className='flex flex-col items-start'>
                            ...
                            <div className='text-[#ffffff6c] text-[.9rem]'>{item.lastMessage ? item.lastMessage.message : "Start the conversation!"}</div>
                        </div>
                        {(() => {
